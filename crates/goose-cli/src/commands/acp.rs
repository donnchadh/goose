...
impl acp::Agent for GooseAcpAgent {
    ...
    async fn prompt(&self, args: acp::PromptRequest) -> Result<acp::PromptResponse, acp::Error> {
        ...
        SessionManager::add_message(&session_id, &user_message).await?;
        ...
        tokio::spawn(async move {
            ...
            {
                warn!("Failed to generate session name: {}", e);
            }
        });
        ...
        while let Some(event) = stream.next().await {
            ...
            match event {
                Ok(goose::agents::AgentEvent::Message(message)) => {
                    if let Err(e) = SessionManager::add_message(&session_id, &message).await {
                        error!("Failed to write message to session: {}", e);
                    }
